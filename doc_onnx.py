...
CLASSES = [
    ...
    "Header_type2",
    "meter_info_type2",
    "meter_read_tou_type2",
    "meter_read_srm_type2",
    "meter_calc_detail_type2",
    "meter_calc_baht_type2",
    "Power_Outage",
    "meter_read_type2",
]
...
def find_box(file_path):
    ...
    if img is None:
        raise FileNotFoundError(f"ไม่พบไฟล์ภาพตาม 경로: {file_path}")
    ...
    # 4. ถอดรหัสผลลัพธ์ (Post-processing)
    ...
    # 5. กรองกรอบที่ซ้อนทับกันด้วย Non-Maximum Suppression (NMS)
    indices = cv2.dnn.NMSBoxes(
        boxes, confidences, score_threshold=0.3, nms_threshold=0.45
    )

    # 6. จัดรูปแบบการคืนค่า (Return) ออกมาเป็น List of Dictionaries
    results = []
    if len(indices) > 0:
...
if __name__ == "__main__":
    ...
    detections = find_box(invoice_img)
